...
@registry.register_metric("coco_caption_bleu4")
class CoCoCaptionBleu4Metric(BaseMetric):
    """Metric for calculating coco caption accuracy using BLEU4 Score.

    **Key:** ``coco_caption_bleu4``
    """

    def __init__(self):
        super().__init__("coco_caption_bleu4")
        self.required_params = ["__prediction_report__", "__prediction_reference__"]
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        references = model_output.prediction_reference  # references (true captions) for calculating BLEU-4 score
...
@registry.register_metric("segment_mean_ap")
class SegmentMeanAP(BaseMetric):
    """Metric for calculating the segment mean average precision (mAP) using the COCO
    evaluation toolkit, returning the default COCO-style mAP@IoU=0.50:0.95
    ...
    **Key:** ``segment_mean_ap``
    ...
    def __init__(self, dataset_json_files, *args, **kwargs):
        """Initialization function segment mean AP (mAP)

        Args:
            dataset_json_files (Dict): paths to the dataset (instance) json files
                for each dataset type and dataset name in the following format:
                ``{'val/segment_coco': '/path/to/instances_val2017.json', ...}``

        """
        super().__init__("segment_mean_ap")
        self.required_params = ["__prediction_report__"]
        self.dataset_json_files = dataset_json_files
    ...
    def calculate(
        ...
        if execute_on_master_only and not is_master():
            ...
            results = model_output.prediction_report
            mAP = torch.tensor(results, dtype=torch.float, device=device)
