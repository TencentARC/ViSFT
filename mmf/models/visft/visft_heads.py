...
from mmf.models.visft.backbone import build_visft_convnet_backbone
from mmf.models.visft.matcher import HungarianMatcher
from mmf.models.visft.misc import NestedTensor
from mmf.models.visft.transformer import DetrTransformer
from mmf.utils import box_ops
from mmf.utils.distributed import get_world_size, is_dist_initialized
from torch import nn, Tensor
from torch.nn.utils.rnn import pack_padded_sequence
from mmf.models.visft.mask_former_head import MaskFormerHead
from mmf.models.visft.caption_decoder import DecoderWithAttention
from mmf.models.visft.msdeformattn import MSDeformAttnPixelDecoder
from mmf.models.visft.mask2former_transformer_decoder import MultiScaleMaskedTransformerDecoder
from mmf.models.visft.misc import ShapeSpec

class ViSFTHeads(nn.Module):
    def __init__(self, args):
        super().__init__()

        self.num_queries = args.num_queries
        self.backbone = build_visft_convnet_backbone(args)
        # Detr Det
        self.transformer = DetrTransformer(args)
        ...
        # args for caption
        attention_dim = 384  # dimension of attention linear layers for caption
        emb_dim = 384  # dimension of word embeddings for caption
        decoder_dim = 384  # dimension of decoder RNN
        encoded_image_size=14
        # Resize image to fixed size to allow input images of variable size
        self.caption_adaptive_pool = nn.AdaptiveAvgPool2d((encoded_image_size, encoded_image_size))
        self.caption_decoder = DecoderWithAttention(attention_dim=attention_dim,
                                       embed_dim=emb_dim,
                                       decoder_dim=decoder_dim,
                                       vocab_size=9490,
                                       encoder_dim=self.backbone.num_channels,
                                       dropout=0.5)
        # args for seg
        self.backbone.output_shape = {
                'res5': ShapeSpec(channels=self.backbone.num_channels, stride=32)
                }
        self.segment_pixel_decoder = MSDeformAttnPixelDecoder(
            input_shape={
                k: v for k, v in self.backbone.output_shape.items()
            },
            conv_dim=256,
            mask_dim=256,
            norm="GN",
        ...
        self.segment_predictor = MultiScaleMaskedTransformerDecoder(
            in_channels=256,
        ...
        self.segment_head = MaskFormerHead(
            input_shape={
                k: v for k, v in self.backbone.output_shape.items()
            },
            ignore_value=255,
            num_classes=80,
            pixel_decoder=self.segment_pixel_decoder,
            loss_weight=1.0,
            transformer_in_feature="multi_scale_pixel_decoder",
            transformer_predictor= self.segment_predictor,
        )
    def forward(
        self,
        img_src: Tensor,
        ...
        caps: Optional[Tensor] = None,
        caplens: Optional[Tensor] = None,
    ):
        img_mask = None
        img_pos = [None]
        ...
        if task_type == "detection":
            ...
            out = self.caption_adaptive_pool(img_src)  # (batch_size, 2048, encoded_image_size, encoded_image_size)
            out = out.permute(0, 2, 3, 1)  # (batch_size, encoded_image_size, encoded_image_size, 2048)
            scores, caps_sorted, decode_lengths, alphas, sort_ind = self.caption_decoder(out, caps, caplens)
            ...
            return {
                    ...
                    "targets": targets,
                    "alphas": alphas,
                    "decode_lengths": torch.LongTensor(decode_lengths).to(torch.device("cuda")),
                    "sort_ind": sort_ind,
                    }
        elif task_type == "segment":
            outputs = self.segment_head(img_src)
            return outputs
