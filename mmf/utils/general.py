...
def print_model_parameters(model, return_only=False):
    ...
    backbone_params = sum(p.numel() for name, p in model.named_parameters() if 'backbone' in name and 'lora' not in name)
    ...
    if not return_only:
        logger.info(
            f"Total Parameters: {total_params /1e6}M. Trained Parameters: {trained_params/1e6}M. Backbone Parameters: {backbone_params/1e6}M. LoRA Parameters: {lora_params/1e6}M. Head Parameters: {head_params/1e6}M."
        )
